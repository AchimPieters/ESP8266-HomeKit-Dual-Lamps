...
void button_callback(uint8_t gpio) {
...
homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id=1, .category=homekit_accessory_category_lightbulb, .services=(homekit_service_t*[]){
        HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics=(homekit_characteristic_t*[]){
            ...
            HOMEKIT_CHARACTERISTIC(SERIAL_NUMBER, "G79R96G6PKMK"),
            HOMEKIT_CHARACTERISTIC(MODEL, "HKES2LL/G"),
...
void create_accessory_name() {
    ...
    int name_len = snprintf(NULL, 0, "Dual Lamp-%02X%02X%02X",macaddr[3], macaddr[4], macaddr[5]);
    char *name_value = malloc(name_len+1);
    snprintf(name_value, name_len+1, "Dual Lamp-%02X%02X%02X",macaddr[3], macaddr[4], macaddr[5]);
